...
(defvar mk-proj-ignore-directorys nil)

(defvar mk-proj-search-directorys nil)
...
(defconst mk-proj-proj-vars '(mk-proj-name
                              ...
                              mk-proj-ignore-directorys
                              mk-proj-search-directorys
                              mk-proj-ack-args
                              mk-proj-vcs
                              mk-proj-tags-file
                              mk-proj-compile-cmd
...
(defun mk-proj-load-vars (proj-name proj-alist)
  ...
  (labels ((config-val (key)
    ...
    ;; optional vars
    (dolist (v '(src-patterns ignore-patterns ignore-directorys search-directorys ack-args vcs
...
(defun mk-proj-find-cmd-ignore-directorys (ignore-directorys)
  "Generate the -not ( -path <path1> -o -path <path2> ...) pattern for find cmd"
  (if ignore-directorys
      (let ((name-expr " -not \\( "))
        (dolist (pat ignore-directorys)
          (setq name-expr (concat name-expr " -path \"" pat "\" -o ")))
        (concat (mk-proj-replace-tail name-expr "-o " "") "\\) "))
    ""))

(defun mk-proj-find-cmd-search-directorys (search-directorys)
  "Generate the ( -path <path1> -o -path <path2> ...) pattern for find cmd"
  (if search-directorys
      (let ((name-expr " \\( "))
        (dolist (pat search-directorys)
          (setq name-expr (concat name-expr " -path \"" pat "\" -o ")))
        (concat (mk-proj-replace-tail name-expr "-o " "") "\\) "))
    ""))
...
C-u prefix, start from the current directory."
  ...
  (let* ((wap (word-at-point))
    ...
    (when mk-proj-ignore-directorys
      (setq find-cmd (concat find-cmd (mk-proj-find-cmd-ignore-directorys mk-proj-ignore-directorys))))
    (when mk-proj-search-directorys
      (setq find-cmd (concat find-cmd (mk-proj-find-cmd-search-directorys mk-proj-search-directorys))))
  ...
  (when mk-proj-file-list-cache
    ...
    (let* ((default-directory (file-name-as-directory mk-proj-basedir))
           ...
           (find-cmd (concat "find '" start-dir "' -type f "
                            ...
                            (mk-proj-find-cmd-ignore-directorys mk-proj-ignore-directorys)
                            (mk-proj-find-cmd-search-directorys mk-proj-search-directorys)))
